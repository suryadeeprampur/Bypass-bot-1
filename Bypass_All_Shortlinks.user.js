...
// @include /^(https?:\/\/)(.+)?(stfly.cc|stfly.xyz|techtrendmakers.com|(exploreera|blogbux|blogesque).net)(\/.*)/
...
'use strict';
  ...
  BypassedByBloggerPemula(/sakarnewz.com/, {'/multi/': [RexBp, 'http://multiquality.xyz/download/'],}, false);//rareanimes.net 1st choice
  ...
  function onHtmlLoaded() {
    ...
    BypassedByBloggerPemula(/stfly.cc|stfly.xyz|techtrendmakers.com|(exploreera|blogbux|blogesque).net/, function() {if (elementExists('.g-recaptcha') || elementExists('.h-captcha')) {
...
(function() {
    ...
    const redirectIfNotDisabled = (selector) => { let intervalId = setInterval(() => { let linkButton = document.querySelector(selector + ':not(.disabled)'); if (linkButton) { clearInterval(intervalId); setTimeout(function() {redirect(linkButton.href);}, 500) } }, 500); };
